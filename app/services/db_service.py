...
class DatabaseService:
    def is_duplicate_webhook(self, webhook_event_id: str) -> bool:
        ...
        with connection() as conn:
            row = conn.execute(
                "SELECT webhook_event_id FROM webhook_events WHERE webhook_event_id = ?",
                (webhook_event_id,),
            ).fetchone()
    ...
    def save_webhook_event(self, webhook_event_id: str, event_type: str, raw_payload: str) -> None:
        ...
        with connection() as conn:
            conn.execute(
                "INSERT OR IGNORE INTO webhook_events (webhook_event_id, event_type, raw_payload) VALUES (?, ?, ?)",
                (webhook_event_id, event_type, raw_payload),
            )
    ...
    def add_log(self, level: str, tag: str, message: str) -> None:
        ...
        with connection() as conn:
            conn.execute(
                "INSERT INTO system_logs (log_time, level, tag, message) VALUES (?, ?, ?, ?)",
                (now_text, level.upper(), tag, message[:4000]),
            )

    def upsert_signup(self, uid: str, name: str, group_id: str, shift: str, signup_at: str, window: str, work_date: str) -> str:
        with connection() as conn:
            existing = conn.execute(
                "SELECT id FROM signups WHERE uid = ? AND work_date = ?",
                (uid, work_date),
            ).fetchone()
            if existing:
                conn.execute(
    ...
    def get_shift_for_date(self, uid: str, work_date: str) -> str:
        with connection() as conn:
            row = conn.execute(
                "SELECT shift FROM signups WHERE uid = ? AND work_date = ?",
                (uid, work_date),
            ).fetchone()
    ...
    def get_attendance_record(self, uid: str, date_key: str) -> Optional[dict[str, Any]]:
        with connection() as conn:
            row = conn.execute(
                "SELECT * FROM attendance_records WHERE uid = ? AND date = ?",
                (uid, date_key),
            ).fetchone()
    ...
    def create_attendance_record(self, date_key: str, uid: str, name: str, group_id: str, shift: str) -> None:
        with connection() as conn:
            conn.execute(
                "INSERT OR IGNORE INTO attendance_records (date, uid, name, group_id, shift) VALUES (?, ?, ?, ?, ?)",
                (date_key, uid, name, group_id, shift),
            )
    ...
    def upsert_attendance_record(
        ...
        with connection() as conn:
            existing = conn.execute(
                "SELECT id FROM attendance_records WHERE uid = ? AND date = ?",
                (uid, date_key),
            ).fetchone()
            if existing:
                conn.execute(
    ...
    def add_attendance_event(self, uid: str, group_id: str, work_date: str, action: str, event_time: str) -> None:
        with connection() as conn:
            conn.execute(
                "INSERT INTO attendance_events (uid, group_id, work_date, action, event_time) VALUES (?, ?, ?, ?, ?)",
                (uid, group_id, work_date, action, event_time),
            )
    ...
    def list_signups(self, target_date: str) -> list[dict[str, Any]]:
        with connection() as conn:
            rows = conn.execute(
                "SELECT id, uid, name, group_id, shift, signup_at, window, status FROM signups WHERE work_date = ? ORDER BY signup_at DESC",
                (target_date,),
            ).fetchall()
        return [
            {
    ...
    def list_logs(self, limit: int = 30) -> list[dict[str, Any]]:
        with connection() as conn:
            rows = conn.execute(
                "SELECT log_time, level, tag, message FROM system_logs ORDER BY id DESC LIMIT ?",
                (limit,),
            ).fetchall()
        ...
        with connection() as conn:
            ...
            for r in rules:
                conn.execute(
                    ...
                    (
                        r.get("rule_date") or "",
                        r.get("source_shift") or "",
                        r.get("shift_alias") or "",
                        r.get("bonus_per_hour"),
    ...
    def update_employee_role(self, uid: str, role: str) -> None:
        with connection() as conn:
            conn.execute(
                "UPDATE employees SET role = ?, updated_at = CURRENT_TIMESTAMP WHERE uid = ?",
                (role, uid),
            )
    ...
    def update_employee_status(self, uid: str, status: str) -> None:
        with connection() as conn:
            conn.execute(
                "UPDATE employees SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE uid = ?",
                (status, uid),
            )
    ...
    def update_employee_note(self, uid: str, note: str) -> None:
        with connection() as conn:
            conn.execute(
                "UPDATE employees SET note = ?, updated_at = CURRENT_TIMESTAMP WHERE uid = ?",
                (note, uid),
            )