...
from app.utils.sheet_parsers import normalize_header_map, parse_datetime_like, signup_datetime_to_work_date
...
class SignupService:
    ...
    def upsert_signup(self, uid: str, name: str, group_id: str, raw_shift: str) -> dict:
        ...
        action = self.db.upsert_signup(uid=uid, name=name, group_id=group_id, shift=shift_text, signup_at=now_text, window='21:00 - 00:00', work_date=work_date)
        logger.info('signup %s uid=%s name=%s shift=%s', action, uid, name, shift_text)
        self.db.add_log('INFO', 'upsertSignup', f'UID={uid} / 姓名={name} / 班別={shift_text} / action={action}')
    ...
    def import_signup_sheet(self) -> int:
        ...
        for row in values[1:]:
            ...
            if not work_date:
                work_date = (now_taipei() + timedelta(days=1)).strftime('%Y-%m-%d')
                signup_at = signup_at or now_taipei().strftime('%Y-%m-%d %H:%M:%S')
            result = self.db.import_signup(uid=uid, name=name or f'使用者-{uid[:6]}', shift=shift, signup_at=signup_at or f'{work_date} 00:00:00', work_date=work_date)
            if result != 'skip':
                count += 1
        return count
